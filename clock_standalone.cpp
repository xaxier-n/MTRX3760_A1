...
// Built as a separate class, so all of the clock behaviour is repeated here,
// with the alarm added on top.
class CAlarmClock
{
  public:
    ...
    // SetAlarm sets the alarm to go off at the given time, in minutes since midnight.
    void SetAlarm( int aAlarmMinutes );

    // IsRinging returns true if the alarm is set to go off at the current time.
    bool IsRinging();

    // Report prints the clock's name and current time as HH:MM. If the alarm
    // is set to go off at this time, it also prints "ALARM!".
    void Report();
...
int main()
{
  ...
  alarm.SetAlarm( 430 );  // set alarm for 07:00
...
{
  return mAlarmSet && ( mCurrentMinutes >= mAlarmMinutes );
}
